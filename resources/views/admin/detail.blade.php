        ...
        <div class="max-w-4xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="bg-white dark:bg-gray-800 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-700 overflow-hidden">
                ...
                <div class="p-6 space-y-8">
                    ...
                    <div class="space-y-4 pt-6 border-t border-gray-100 dark:border-gray-800">
                        <h4 class="text-sm font-bold text-gray-400 uppercase tracking-wider">Nomor Resi & Timer Pengiriman</h4>
                        <div class="bg-gray-50 dark:bg-gray-900/50 border border-gray-100 dark:border-gray-800 rounded-xl p-5 space-y-4">
                            <div class="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
                                <div>
                                    <span class="text-xs text-gray-400 block">Nomor Resi Pengiriman</span>
                                    <span class="text-base font-bold font-mono text-teal-400">{{ $order->no_resi ?? 'Belum diisi oleh Seller' }}</span>
                                </div>

                                <form action="{{ route('admin.order.updateResi', $order->id) }}" method="POST" class="flex items-center gap-2">
                                    @csrf
                                    <input type="text" name="no_resi" value="{{ $order->no_resi }}" placeholder="Update No. Resi" required class="text-xs bg-slate-950 border border-slate-700 rounded-xl px-3 py-2 text-white focus:border-teal-500 w-44" />
                                    <button type="submit" class="px-3 py-2 bg-indigo-600 hover:bg-indigo-500 text-white rounded-xl text-xs font-bold transition">Update Resi</button>
                                </form>
                            </div>
                            ...
                            <div class="pt-4 border-t border-gray-200 dark:border-gray-800 space-y-4">
                                <div class="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
                                    <div>
                                        <span class="text-xs font-bold text-gray-300 block">Status Monitoring Pengiriman Admin</span>
                                        ...
                                        @elseif($order->waktu_sampai)
                                            ...
                                            <p class="text-xs text-amber-400 font-medium mt-0.5">
                                                Waktu Sampai: {{ \Carbon\Carbon::parse($order->waktu_sampai)->format('d M Y, H:i') }} WIB 
                                                @if(!$isExpired)
                                        ...
                                        @else
                                            <p class="text-xs text-gray-400 mt-0.5">Pantau resi kurir. Tekan tombol sesuai status terupdate dari ekspedisi.</p>
                                        @endif
                                    </div>

                                    <div class="flex flex-wrap items-center gap-2">
                                        @if($order->status === 'dikirim' || $order->status === 'keterlambatan')
                                            <form action="{{ route('admin.order.markInvestigation', $order->id) }}" method="POST" onsubmit="return confirm('Tandai order ini sebagai BARANG HILANG / INVESTIGASI?');">
                                                @csrf
                                                <button type="submit" class="px-3 py-2 bg-rose-900/80 hover:bg-rose-800 text-rose-200 border border-rose-700 rounded-xl text-xs font-bold transition">
                                        ...
                                        @if(!$order->waktu_sampai && ($order->barang_dikirim || $order->status === 'dikirim' || $order->status === 'keterlambatan'))
                                            <form action="{{ route('admin.order.markArrived', $order->id) }}" method="POST">
                                                @csrf
                                                <button type="submit" onclick="return confirm('Mulai timer 24 jam konfirmasi untuk buyer?');" class="px-3.5 py-2 bg-amber-600 hover:bg-amber-500 text-white rounded-xl text-xs font-bold transition">
                                        ...
                                        @if(!in_array($order->status, ['selesai', 'dibatalkan']))
                                            <form action="{{ route('admin.order.autoConfirm', $order->id) }}" method="POST">
                                                @csrf
                                                <button type="submit" onclick="return confirm('Selesaikan transaksi dan teruskan saldo ke seller?');" class="px-3.5 py-2 bg-emerald-600 hover:bg-emerald-500 text-white rounded-xl text-xs font-bold transition">
                                ...
                                @if($order->status === 'investigasi')
                                    <div class="bg-rose-950/60 border border-rose-500/40 rounded-xl p-4 space-y-3">
                                        <h5 class="text-xs font-black text-rose-300 uppercase tracking-wider">Hasil Verifikasi & Investigasi Ekspedisi:</h5>
                                        <p class="text-xs text-gray-300 leading-relaxed">
                                            Admin mengecek bukti resi & verifikasi dengan pihak ekspedisi. Apakah barang ditemukan atau dipastikan hilang?
                                        </p>
                                        <div class="flex flex-col sm:flex-row gap-3 pt-2">
                                            <form action="{{ route('admin.order.resolveInvestigation', $order->id) }}" method="POST" class="flex-1">
                                                @csrf
                                                <input type="hidden" name="hasil_investigasi" value="ditemukan">
                                                <button type="submit" onclick="return confirm('Barang ditemukan? Ekspedisi akan mengirim ulang ke Buyer.')" class="w-full py-2 bg-emerald-600 hover:bg-emerald-500 text-white text-xs font-bold rounded-lg transition">
                                            ...
                                            <form action="{{ route('admin.order.resolveInvestigation', $order->id) }}" method="POST" class="flex-1">
                                                ...
                                                <button type="submit" onclick="return confirm('Barang pasti hilang? Dana akan dikembalikan (Refund) ke Buyer dan pesanan dibatalkan.')" class="w-full py-2 bg-rose-700 hover:bg-rose-600 text-white text-xs font-bold rounded-lg transition">
                    ...
                    @if($order->status === 'komplain' || $order->alasan_komplain)
                        <div class="space-y-4 pt-6 border-t border-rose-500/30">
                            <h4 class="text-sm font-bold text-rose-400 uppercase tracking-wider">⚠️ Peninjauan Pengajuan Refund / Komplain Buyer</h4>
                            <div class="bg-rose-500/10 border border-rose-500/30 rounded-2xl p-5 space-y-4">
                                <div>
                                    ...
                                    <p class="text-sm text-gray-200 mt-1 bg-slate-950 p-3 rounded-xl border border-slate-800">{{ $order->alasan_komplain }}</p>
                                </div>

                                <div>
                                    <span class="text-xs text-rose-300 font-bold block mb-2">Bukti Video Unboxing (Wajib):</span>
                                    @if($order->video_unboxing)
                                        <div class="max-w-md rounded-xl overflow-hidden border border-slate-800 bg-slate-950 p-2">
                                            <video controls class="w-full h-auto rounded-lg">
                                                <source src="{{ asset('storage/unboxing_videos/' . $order->video_unboxing) }}">
                                                Browser Anda tidak mendukung pemutar video.
                                            </video>
                                            <a href="{{ asset('storage/unboxing_videos/' . $order->video_unboxing) }}" target="_blank" class="text-xs text-indigo-400 hover:underline block mt-2 text-center">
                                ...
                                @if($order->no_resi_retur || $order->norek_refund)
                                    <div class="bg-indigo-950/90 border-2 border-indigo-500/60 rounded-2xl p-5 space-y-3 my-4 shadow-xl">
                                        <div class="flex items-center justify-between">
                                            <h5 class="text-xs font-black text-indigo-300 uppercase tracking-wider flex items-center gap-2">
                                                📦 Data Resi Pengembalian & Rekening Refund Buyer:
                                            </h5>
                                            <span class="px-2.5 py-0.5 rounded-full text-[10px] font-black bg-emerald-500/20 text-emerald-300 border border-emerald-500/40">
                                                TERISI OLEH BUYER ✓
                                            </span>
                                        </div>
                                        ...
                                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-3 text-xs">
                                            <div class="bg-slate-900 p-3 rounded-xl border border-indigo-500/30">
                                                <span class="text-gray-400 block text-[10px] uppercase font-bold mb-1">Resi Retur Pengembalian Barang:</span>
                                                @if($order->no_resi_retur)
                                                    <div class="flex items-center gap-2">
                                                        <span class="text-xs px-2 py-0.5 bg-indigo-500/30 text-indigo-200 font-bold rounded border border-indigo-500/40">
                                                            {{ $order->ekspedisi_retur ?? 'Ekspedisi' }}
                                                        </span>
                                                        <span class="font-mono font-bold text-teal-300 text-sm bg-slate-950 px-2 py-0.5 rounded border border-slate-800">
                                                            {{ $order->no_resi_retur }}
                                                        </span>
                                                    </div>
                                                    <span class="text-[10px] text-gray-400 block mt-1">Tanggal Retur: {{ $order->tanggal_retur ? $order->tanggal_retur->diffForHumans() : '-' }}</span>
                                                @else
                                                    <span class="text-amber-400 italic font-semibold">Buyer Belum Menginput Resi Retur</span>
                                                @endif
                                            </div>

                                            <div class="bg-slate-900 p-3 rounded-xl border border-indigo-500/30">
                                                <span class="text-gray-400 block text-[10px] uppercase font-bold mb-1">Rekening Refund Buyer (Tujuan Transfer):</span>
                                                @if($order->norek_refund)
                                                    <span class="font-bold text-white block text-sm">{{ $order->bank_refund }} - <strong class="text-amber-300 font-mono">{{ $order->norek_refund }}</strong></span>
                                                    <span class="text-xs text-indigo-300 font-semibold block mt-0.5">a.n {{ $order->namarek_refund }}</span>
                                                @else
                                                    <span class="text-amber-400 italic font-semibold">Buyer Belum Menginput Rekening</span>
                                                @endif
                                            </div>
                                        </div>
                                    </div>
                                ...
                                @if($order->status === 'komplain')
                                    <div class="pt-4 border-t border-rose-500/20 space-y-4">
                                        ...
                                        <div class="bg-amber-500/10 border border-amber-500/30 rounded-xl p-4 space-y-2">
                                            <h5 class="text-xs font-black text-amber-300 uppercase tracking-wider flex items-center gap-1.5">
                                            ...
                                            <div class="grid grid-cols-1 sm:grid-cols-2 gap-2 text-xs text-gray-300">
                                                <div>
                                                    ...
                                                    <span class="font-black text-teal-300 block text-sm">Rp {{ number_format($order->total_harga, 0, ',', '.') }}</span>
                                                </div>
                                                <div>
                                                    <span class="text-gray-400">Penerima Refund (Buyer):</span>
                                                    <span class="font-bold text-white block">{{ $order->nama ?? $order->user->name }} ({{ $order->email ?? $order->user->email }})</span>
                                                </div>
                                            </div>
                                        </div>

                                        <p class="text-xs text-gray-400 leading-relaxed">
                                            Tinjau alasan dan video unboxing dari buyer. Jika disetujui, buyer akan diminta mengembalikan barang ke seller & menginput nomor rekening refund.
                                        </p>
                                        <div class="flex flex-col sm:flex-row gap-3">
                                            <form action="{{ route('admin.order.approveRefund', $order->id) }}" method="POST" class="flex-1" onsubmit="return confirm('Setujui komplain buyer? Buyer akan diminta memasukkan No. Resi Retur & Rekening Refund.')">
                                                @csrf
                                                <button type="submit" class="w-full py-2.5 bg-amber-600 hover:bg-amber-700 text-white rounded-xl text-xs font-black uppercase tracking-wider transition duration-150 shadow-md">
                                                    ✅ Setujui Refund (Minta Buyer Kirim Barang & No. Rekening)
                                                </button>
                                            </form>
                                            <form action="{{ route('admin.order.rejectRefund', $order->id) }}" method="POST" class="flex-1" onsubmit="return confirm('Tolak refund? Transaksi akan diselesaikan dan saldo diteruskan ke seller.')">
                                                @csrf
                                                <button type="submit" class="w-full py-2.5 bg-rose-700 hover:bg-rose-800 text-white rounded-xl text-xs font-black uppercase tracking-wider transition duration-150 shadow-md">
                                ...
                                @elseif(in_array($order->status, ['refund_disetujui', 'barang_diretur']))
                                    ...
                                    <div class="pt-4 border-t border-indigo-500/30 space-y-4">
                                        <div class="bg-indigo-950/80 border border-indigo-500/40 rounded-2xl p-5 space-y-3">
                                            <h5 class="text-xs font-black text-indigo-300 uppercase tracking-wider flex items-center gap-1.5">
                                                📦 Status Pengembalian Barang & Rekening Buyer:
                                            </h5>
                                            
                                            <div class="grid grid-cols-1 sm:grid-cols-2 gap-3 text-xs">
                                                <div class="bg-slate-900 p-3 rounded-xl border border-slate-800">
                                                    <span class="text-gray-400 block text-[10px] uppercase font-bold mb-0.5">Resi Retur Pengembalian Barang:</span>
                                                    @if($order->no_resi_retur)
                                                        <span class="inline-flex items-center gap-2 mt-0.5">
                                                            ...
                                                            <span class="font-mono font-bold text-teal-300 text-sm">{{ $order->no_resi_retur }}</span>
                                                        </span>
                                                        <span class="text-[10px] text-gray-400 block mt-1">Dikirim {{ $order->tanggal_retur ? $order->tanggal_retur->diffForHumans() : '' }}</span>
                                                    @else
                                                        <span class="text-amber-400 italic font-semibold">Buyer Belum Menginput Resi Retur</span>
                                                    @endif
                                                </div>
                                                ...
                                                <div class="bg-slate-900 p-3 rounded-xl border border-slate-800">
                                                    <span class="text-gray-400 block text-[10px] uppercase font-bold mb-0.5">Rekening Refund Buyer:</span>
                                                    @if($order->norek_refund)
                                                        <span class="font-bold text-white block">{{ $order->bank_refund }} - {{ $order->norek_refund }}</span>
                                                        <span class="text-xs text-indigo-300">a.n {{ $order->namarek_refund }}</span>
                                                    @else
                                                        <span class="text-amber-400 italic font-semibold">Buyer Belum Menginput Rekening</span>
                                                    @endif
                                                </div>
                                            ...
                                            <div class="pt-2">
                                                @if($order->retur_diterima_seller)
                                                    <p class="text-xs text-emerald-400 font-bold mb-3 flex items-center gap-1">
                                                        ✓ Seller telah mengonfirmasi bahwa barang retur sudah sampai di toko/rumahnya! Silakan transfer dana refund ke rekening buyer.
                                                    </p>
                                                @else
                                                    <p class="text-xs text-amber-400 mb-3 leading-relaxed">
                                                        ⏳ Barang dalam perjalanan kembali ke Seller. Jika Seller sudah mengonfirmasi terima barang (atau resi retur menunjukkan paket tiba), Anda dapat memproses transfer refund di bawah.
                                                    </p>
                                                @endif

                                                @if($order->norek_refund)
                                                    {{-- FORM UPLOAD BUKTI TRANSFER REFUND --}}
                                                    <form action="{{ route('admin.order.finalizeRefund', $order->id) }}" method="POST" enctype="multipart/form-data" onsubmit="return confirm('Konfirmasi bahwa Anda sudah mentransfer Rp {{ number_format($order->total_harga, 0, ',', '.') }} ke rekening Buyer?');">
                                                        @csrf
                                                        @if($errors->has('bukti_transfer_refund'))
                                                            <div class="mb-2 p-2 bg-rose-500/20 border border-rose-500/40 rounded-lg text-rose-300 text-xs">
                                                                {{ $errors->first('bukti_transfer_refund') }}
                                                            </div>
                                                        @endif
                                                        <div class="space-y-2">
                                                            <label class="block text-[10px] font-bold text-gray-400 uppercase tracking-wider">📸 Upload Screenshot / Foto Bukti Transfer Refund (Wajib)</label>
                                                            <input type="file" name="bukti_transfer_refund" accept="image/*" required
                                                            ...
                                                            <p class="text-[10px] text-gray-500">Format: JPG/PNG/WEBP, maks. 5 MB</p>
                                                        </div>
                                                        <button type="submit" class="w-full py-3 bg-emerald-600 hover:bg-emerald-500 text-white rounded-xl text-xs font-black uppercase tracking-wider transition shadow-lg shadow-emerald-600/20">
                                                            💸 Konfirmasi Transfer & Upload Bukti — Rp {{ number_format($order->total_harga, 0, ',', '.') }}
                                                        </button>
                                                    </form>
                                                @endif
                                            </div>
                                ...
                                @elseif($order->status === 'menunggu_konfirmasi_refund')
                                    ...
                                    <div class="pt-4 border-t border-cyan-500/20">
                                        <div class="bg-cyan-950/60 border border-cyan-500/40 rounded-2xl p-5 space-y-3">
                                            <h5 class="text-xs font-black text-cyan-300 uppercase tracking-wider flex items-center gap-1.5">
                                            ...
                                            @if($order->bukti_transfer_refund)
                                                <div class="p-2 border border-cyan-500/30 bg-slate-950 rounded-xl max-w-xs">
                                                    <p class="text-[10px] text-gray-400 mb-1.5 font-bold uppercase">Screenshot Bukti Transfer Admin:</p>
                                                    <a href="{{ asset('storage/bukti_refund/' . $order->bukti_transfer_refund) }}" target="_blank">
                                                        <img src="{{ asset('storage/bukti_refund/' . $order->bukti_transfer_refund) }}" class="w-full rounded-lg hover:opacity-80 transition border border-slate-700" alt="Bukti Transfer Refund">
                                                    </a>
                                                    <a href="{{ asset('storage/bukti_refund/' . $order->bukti_transfer_refund) }}" target="_blank" class="text-[10px] text-cyan-400 hover:underline block text-center mt-1">Buka di tab baru</a>
                                                </div>
                                            @endif
                                            <p class="text-xs text-gray-300 leading-relaxed">
                                                Admin telah mengklaim sudah mentransfer dana refund sebesar <strong class="text-teal-300">Rp {{ number_format($order->total_harga, 0, ',', '.') }}</strong> ke rekening Buyer ({{ $order->bank_refund }} - {{ $order->norek_refund }} a.n {{ $order->namarek_refund }}).
                                ...
                                @elseif($order->status === 'dibatalkan' && $order->alasan_komplain)
                                    <div class="pt-4 border-t border-rose-500/20">
                                        <div class="bg-rose-950/80 border border-rose-500/40 rounded-xl p-4 space-y-2">
                                            ...
                                            <p class="text-xs text-gray-300 leading-relaxed">
                                                Pengembalian dana sebesar <strong class="text-teal-300">Rp {{ number_format($order->total_harga, 0, ',', '.') }}</strong> ke rekening Buyer ({{ $order->bank_refund }} - {{ $order->norek_refund }} a.n {{ $order->namarek_refund }}) telah dikonfirmasi diterima.
                                            </p>
                                        </div>
                                    </div>
                                @endif
                    ...
                    <div class="space-y-4 pt-6 border-t border-indigo-500/20">
                        <h4 class="text-sm font-bold text-indigo-400 uppercase tracking-wider flex items-center gap-2">
                        ...
                        <div class="bg-indigo-500/10 border border-indigo-500/30 rounded-2xl p-4 flex items-start gap-3">
                            <div class="w-8 h-8 rounded-full bg-indigo-500/20 flex items-center justify-center shrink-0 mt-0.5">
                            ...
                            <div class="flex-1 min-w-0">
                                <p class="text-xs font-black text-indigo-300 uppercase tracking-wider mb-1">Pesan Terakhir Dikirim ke Buyer:</p>
                                <p class="text-sm text-gray-200 leading-relaxed">{{ $order->admin_note }}</p>
                                <div class="flex items-center gap-4 mt-2 flex-wrap">
                                    @if($order->estimasi_tiba)
                                        <span class="inline-flex items-center gap-1.5 text-xs font-bold text-teal-400 bg-teal-500/10 border border-teal-500/20 px-2.5 py-1 rounded-full">
                                            📅 Estimasi Tiba: {{ \Carbon\Carbon::parse($order->estimasi_tiba)->locale('id')->isoFormat('D MMMM YYYY') }}
                                        </span>
                                    @endif
                                    @if($order->admin_note_at)
                                        <span class="text-[11px] text-gray-500">Dikirim {{ $order->admin_note_at->diffForHumans() }}</span>
                                    @endif
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="bg-gray-50 dark:bg-gray-900/30 border border-indigo-500/20 rounded-2xl p-5">
                            <p class="text-xs text-gray-400 mb-4 leading-relaxed">
                                Gunakan fitur ini untuk memberi tahu buyer mengenai kondisi pengiriman, keterlambatan, atau informasi lain yang perlu diketahui buyer. Pesan ini akan muncul <strong class="text-indigo-300">secara mencolok</strong> di halaman riwayat pembelian buyer.
                            </p>
                            <form action="{{ route('admin.order.sendNote', $order->id) }}" method="POST" class="space-y-4">
                                @csrf
                                <div>
                                    ...
                                    <div class="flex justify-between mt-1">
                                        <span class="text-[11px] text-gray-500">Tulis pesan yang jelas, sopan, dan meyakinkan agar buyer tetap tenang.</span>
                                        <span id="charCount" class="text-[11px] text-gray-500">{{ strlen($order->admin_note ?? '') }}/1000</span>
                                    </div>
                                </div>
                                ...
                                <div>
                                    ...
                                    <input
                                        type="date"
                                        name="estimasi_tiba"
                                        value="{{ $order->estimasi_tiba ? $order->estimasi_tiba->format('Y-m-d') : '' }}"
                                        min="{{ date('Y-m-d') }}"
                                    ...
                                    />
                                    <p class="text-[11px] text-gray-500 mt-1">Kosongkan jika belum ada estimasi yang pasti.</p>
                                </div>

                                <div class="flex justify-end pt-1">
                                    <button type="submit" class="flex items-center gap-2 px-5 py-2.5 bg-indigo-600 hover:bg-indigo-500 text-white rounded-xl text-sm font-bold transition shadow-lg shadow-indigo-600/20">
