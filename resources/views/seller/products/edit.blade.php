                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div>
                            ...
                            <div class="grid grid-cols-2 md:grid-cols-5 gap-4">
                                @for($i = 0; $i < 10; $i++)
                                    @php
                                        $existingPhoto = isset($product->fotos[$i]) ? asset('storage/products/' . $product->fotos[$i]) : null;
                                    @endphp
                                    <div x-data="{ preview: '{{ $existingPhoto }}' }"
                            ...
                            <p class="text-[11px] text-amber-500/80 mt-4 leading-relaxed">
                                <b>Perhatian:</b> Jika Anda mengunggah ulang <b>satu foto baru</b>, Anda wajib mereset
                                seluruh galeri dengan mengunggah komposisi foto secara penuh karena pembaruan bersifat
                                REPLACE keseluruhan. Maks 2MB/file. Biarkan kosong jika tidak ada perubahan galeri
                                apapun.
                            </p>
                        ...
                        <div class="md:col-span-2 border-t border-gray-100 dark:border-gray-700 pt-6">
                            ...
                            <p class="text-xs text-gray-400 mb-4">Ubah rekening M-Banking Anda agar Admin dapat
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                                <div>
                                    ...
                                    <select name="bank_name" id="bank_name"
                                        ...
                                        <option value="M-Banking BCA" {{ old('bank_name', $product->bank_name) === 'M-Banking BCA' ? 'selected' : '' }}>M-Banking BCA
                                        </option>
                                        <option value="M-Banking Mandiri" {{ old('bank_name', $product->bank_name) === 'M-Banking Mandiri' ? 'selected' : '' }}>M-Banking
                                            Mandiri</option>
                                        <option value="M-Banking BNI" {{ old('bank_name', $product->bank_name) === 'M-Banking BNI' ? 'selected' : '' }}>M-Banking BNI
                                        </option>
                                        <option value="M-Banking BRI" {{ old('bank_name', $product->bank_name) === 'M-Banking BRI' ? 'selected' : '' }}>M-Banking BRI
                                        </option>
                                    </select>
                                    @error('bank_name')
                                        <p class="text-sm text-rose-500 mt-2">{{ $message }}</p>
                                ...
                                <div>
                                    ...
                                    <input type="text" name="no_rekening" id="no_rekening"
                                        value="{{ old('no_rekening', $product->no_rekening) }}"
                                        placeholder="Contoh: 7140928122"
                                        class="w-full px-4 py-3 rounded-xl bg-gray-50 border border-gray-200 dark:bg-gray-900 dark:border-gray-700 focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 transition text-gray-800 dark:text-gray-200">
                                    @error('no_rekening')
                                        <p class="text-sm text-rose-500 mt-2">{{ $message }}</p>
                                    @enderror
                                ...
                                <div>
                                    ...
                                    <input type="text" name="atas_nama" id="atas_nama"
                                        value="{{ old('atas_nama', $product->atas_nama) }}"
                        ...
                        <div class="md:col-span-2">
                            ...
                            <textarea name="deskripsi" id="deskripsi" rows="5" required
                                class="w-full px-4 py-3 rounded-xl bg-gray-50 border border-gray-200 dark:bg-gray-900 dark:border-gray-700 focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 transition text-gray-800 dark:text-gray-200">{{ old('deskripsi', $product->deskripsi) }}</textarea>
                            @error('deskripsi')
                                <p class="text-sm text-rose-500 mt-2">{{ $message }}</p>
                            @enderror
                        </div>
