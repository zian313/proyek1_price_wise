            ...
            <div class="flex items-center justify-between gap-4">
                <label for="remember_me" class="inline-flex items-center gap-2 text-sm text-slate-300">
                    <input id="remember_me" type="checkbox"
                        class="rounded border-slate-700 bg-slate-950 text-teal-500 shadow-sm focus:border-teal-500 focus:ring-teal-500"
                        name="remember">
                    {{ __('ingatkan saya') }}
                </label>

                <a class="text-sm text-teal-400 hover:text-teal-300" href="{{ route('password.request') }}">
                    {{ __('Lupa password?') }}
                </a>
            </div>
