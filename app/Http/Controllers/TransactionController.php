    ...
    public function buyerKonfirmasiRefund(Request $request, $id)
    {
        ...
        if ($sudahMasuk) {
            DB::transaction(function () use ($order) {
                ...
                // Kembalikan stok produk karena barang retur sudah diterima seller
                foreach ($order->orderDetails as $detail) {
                    if ($detail->product) {
            ...
            return redirect()->route('orders.history')->with('success', '✅ Refund selesai! Konfirmasi berhasil dan stok produk telah dikembalikan otomatis.');
        } else {
